...
static int32_t Receive_Packet (uint8_t *data, int32_t *length, uint32_t timeout)
{
  ...
  *data = c;
  ...
  for (i = 1; i < (packet_size + PACKET_OVERHEAD); i ++)  //接受数据包剩余的数据，大小：数据区大小+数据头&校验位-1  
  {
  ...
  /* Compute the CRC */   
  computedcrc = Cal_CRC16(&data[PACKET_HEADER], (uint32_t)packet_size);  //从数据区开始，计算数据区的CRC
  ...
  if(computedcrc != (uint16_t)((data[packet_size+3]<<8) | data[packet_size+4])) //比较CRC计算是否一致
  {
    /* CRC error */
    return -1;
  }
      ...
      switch (Receive_Packet(packet_data, &packet_length, NAK_TIMEOUT))
      {
        case 0:
          ...
          switch (packet_length)
          {
            ...
            /* Normal packet */
            default:
              if((packet_data[PACKET_SEQNO_INDEX] & 0xff) != (packets_received & 0xff))  //比较数据包中的 包序号 与本地记录的 包序号 是否相等  
              {
                Send_Byte(NAK);
              }
              else
              {
                ...
                {
                  /* Filename packet */
                  if (packet_data[PACKET_HEADER] != 0)  //数据段第一个字节非0   文件名 可用
                  {
                    /* Filename packet has valid data */   //文件名以 空格符 结束
                ...
                else
                {
                  ...
                  if (STMFLASH_Write(flashdestination, (uint16_t*) ramsource, (uint16_t) packet_length/2)  == 0)
                  {
